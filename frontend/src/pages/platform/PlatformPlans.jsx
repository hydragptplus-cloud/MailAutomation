...
const emptyPlan = {
  slug: "", name: "", price_bdt: 0, email_limit: 0, daily_email_limit: 0,
  weekly_email_limit: 0, max_admins: 1, max_users: 1, max_smtp_accounts: 1,
  max_recipients: 10000, max_campaigns_per_day: 10,
  is_free: false, is_active: true, display_order: 0,
};

const numberFields = [
  ["price_bdt", "Price (BDT)"], ["email_limit", "Emails per 30 days"],
  ["daily_email_limit", "Daily email limit"], ["weekly_email_limit", "Weekly email limit"],
  ["max_admins", "Maximum administrators"], ["max_users", "Maximum users"],
  ["max_smtp_accounts", "SMTP accounts"], ["display_order", "Display order"],
  ["max_recipients", "Maximum recipients"], ["max_campaigns_per_day", "Campaigns per day"],
];
...
export default function PlatformPlans() {
  ...
  function editPlan(plan) { setEditing(plan.id); setForm(Object.fromEntries(Object.keys(emptyPlan).map((key) => [key, plan[key]]))); setModalOpen(true); setError(""); }
  function close() { setModalOpen(false); setEditing(null); setForm(emptyPlan); }
  async function save(event) {
    event.preventDefault(); setSaving(true); setError(""); setMessage("");
    try {
      if (editing) await api.patch(`/billing/platform/plans/${editing}/`, form);
      else await api.post("/billing/platform/plans/", form);
