...
export default function PlatformSessions() {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3"><div className="inline-flex self-start p-1 bg-slate-900 border border-slate-800 rounded-md">{[["active", "Active"], ["revoked", "Revoked"], ["all", "All"]].map(([value, label]) => <button key={value} onClick={() => setFilter(value)} className={`px-3 py-1.5 rounded text-xs font-medium ${filter === value ? "bg-slate-700 text-white" : "text-slate-500 hover:text-slate-200"}`}>{label}</button>)}</div><label className="relative md:w-80"><Search className="absolute left-3 top-3 w-4 h-4 text-slate-500" /><input aria-label="Search sessions" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search user or IP address" className="w-full pl-10" /></label></div>
