...
const METRICS_DATA = [
  { value: "29.4K", label: "Delivered", change: "+12.4%" },
  { value: "98.8%", label: "Success Rate", change: "+0.4%" },
  { value: "842", label: "In Queue", change: "Optimal" },
];

const CHART_BARS = [38, 52, 45, 68, 60, 84, 76, 92, 80, 96, 88, 100];
      ...
      <main className="relative z-10">
        ...
        <section className="max-w-7xl mx-auto px-5 lg:px-8 pt-16 pb-20 lg:pt-28 lg:pb-36 grid lg:grid-cols-[1.15fr_0.85fr] gap-12 lg:gap-16 items-center">
          ...
          <div className="relative">
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-900/70 backdrop-blur-2xl p-6 sm:p-7 shadow-2xl relative">
              ...
              <div className="grid grid-cols-3 gap-3 my-5">
                {METRICS_DATA.map(({ value, label, change }) => (
                  <div
                    key={label}
                    className="rounded-2xl bg-slate-950/70 border border-white/5 p-3.5 text-left"
                  >
                    <span className="text-[10px] text-emerald-400 font-semibold block mb-0.5">{change}</span>
              ...
              {/* Realtime Bar Visualizer */}
              <div className="h-36 flex items-end gap-1.5 px-1 pb-2">
                {CHART_BARS.map((height, i) => (
                  <div
                    key={i}
                    className="flex-1 rounded-t-sm bg-gradient-to-t from-indigo-600/40 via-indigo-400 to-cyan-300 transition-all duration-300 hover:brightness-125 cursor-pointer"
                    style={{ height: `${height}%`, opacity: 0.4 + i * 0.05 }}
                    title={`Bucket ${i + 1}: ${height}%`}
                  />
                ))}
              </div>
              ...
              <div className="mt-4 p-3.5 rounded-2xl bg-indigo-500/10 border border-indigo-400/20 flex items-center gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] sm:text-xs text-slate-400">
                    5 active routes · 0 delivery incidents
                  </p>
                </div>
              </div>
            </div>
