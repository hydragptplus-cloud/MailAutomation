...
export default function Payment() {
  ...
  const customLimits = invoice.snapshot_limits?.custom_plan ? invoice.snapshot_limits : null;
  return <div className="min-h-screen bg-[#070b14] text-slate-100 px-4 py-8 lg:py-14"><div className="max-w-3xl mx-auto">
    ...
    <div className="mt-8 border border-white/10 bg-slate-900/65 overflow-hidden shadow-2xl rounded-md">
      <div className="p-7 sm:p-9 border-b border-white/7 flex flex-col sm:flex-row gap-5 sm:items-center justify-between"><div><p className="text-xs text-indigo-300 font-bold uppercase">USDT invoice</p><div className="flex items-center gap-3 mt-2 flex-wrap"><h1 className="text-3xl font-black">{invoice.plan.name}</h1>{invoice.plan?.discount_percent > 0 && <span className="px-2 py-0.5 rounded-full bg-emerald-400/15 text-emerald-400 text-[10px] font-bold uppercase tracking-wider">{invoice.plan.discount_percent}% Discount applied</span>}</div><p className="text-sm text-slate-500 mt-1">{labels[invoice.network]} · invoice {invoice.id.slice(0, 8)} · ৳{Number(invoice.price_bdt).toLocaleString()}</p></div><span className={`flex items-center gap-2 text-sm ${expired ? "text-rose-300" : "text-amber-300"}`}><Clock3 className="w-4 h-4" /> {expired ? "Quote expired" : `Expires ${new Date(invoice.expires_at).toLocaleString()}`}</span></div>
      <div className="p-7 sm:p-9 space-y-7">
        {customLimits && <div className="rounded-md border border-cyan-400/20 bg-cyan-400/5 p-5 text-sm text-slate-200"><h2 className="font-bold text-cyan-200">Custom plan details</h2><div className="mt-3 grid gap-2 sm:grid-cols-2 text-xs text-slate-400">{[`${Number(customLimits.email_limit).toLocaleString()} emails / 30 days`, `${Number(customLimits.max_admins).toLocaleString()} admins`, `${Number(customLimits.max_users).toLocaleString()} users`, `${Number(customLimits.max_smtp_accounts).toLocaleString()} SMTP + inboxes`, `${Number(customLimits.max_recipients).toLocaleString()} recipients`, `Payable ৳${Number(customLimits.payable_price_bdt || invoice.price_bdt).toLocaleString()}`].map((item) => <span key={item} className="flex gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-cyan-300" />{item}</span>)}</div></div>}
